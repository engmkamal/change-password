...
config: AngularEditorConfig = {
  ...
  ngOnInit(): void {     
    ...
    this.appDataSubscription = this.appdataproviderService.appData$.subscribe((res:any) => {
      if (res.uId != "") {
        ...
        //this.requestorsInfo = this.userService.customerInfo;
  
        // if(Object.prototype.hasOwnProperty(this.requestorsInfo, 'CustId')){
        //   this.executeOnInitProcesses();
        // }else{
  
        //   //let ls = localStorage.getItem('logedCustomer.CustId');
    
        //   //let ls = localStorage.setItem('logedCustomer.CustId', '1000');
    
        //   this.userService.reloadCustomerInfo('1000').then((res:any)=>{
  ...
  async executeOnInitProcesses(){ 
    ...
    // let ustomerRegID = "1000";
    
    // let customerInfo:any = await this.getCustomerInfo(ustomerRegID);

    // if(Object.prototype.hasOwnProperty.call(customerInfo, 'AppParameters')){   
    ...
    try{
      ...
      await this._getSupCatResponsibles();
    ...
    async executeOnInitProcessesOnUid(){    
      try{
        ...
        this.appDataSubscription = this.appdataproviderService.appData$.subscribe((res:any) => {
          this.sapModules = res.sapModules;
          this.otherCategories = res.otherCategories ;

          //===== mappning master data with service and component local veriable start====

          this.supModel.Requestor =                
          {
            CustId: res.Requestor.CustId,
            CustName: res.Requestor.CustName,
            CustCompanyName: res.Requestor.CustCompanyName,
            CustCompany1stAddress: res.Requestor.CustCompany1stAddress,
            Cust1stEmail: res.Requestor.Cust1stEmail,
            Cust1stMobile: res.Requestor.Cust1stMobile,
            CustDesignation: res.Requestor.CustDesignation
          };

          this.supModel.AppParameters.ProblemDescription =                
          {
            RequestFor: res.AppParameters.ProblemDescription.RequestFor,
            RequestCategory: res.AppParameters.ProblemDescription.RequestCategory,
            Subject: res.AppParameters.ProblemDescription.Subject,
            Description: res.AppParameters.ProblemDescription.Description,
          };


          this.supModel.AppParameters.PriorityInfo =                
          {
            Priority: res.AppParameters.PriorityInfo.Priority,
            EmergContact: res.AppParameters.PriorityInfo.EmergContact,
            BusinessImpact: res.AppParameters.PriorityInfo.BusinessImpact,
            EmergContactNumber: res.AppParameters.PriorityInfo.EmergContactNumber,
            EmergContactEmail: res.AppParameters.PriorityInfo.EmergContactEmail
          };

                    
          this.supModel.AppParameters.SystemDetail =                
          {                    
            SystemType: res.AppParameters.SystemDetail.SystemType,
            SystemModule: res.AppParameters.SystemDetail.SystemModule,        
            SAPCustomerNumber: res.AppParameters.SystemDetail.SAPCustomerNumber,
            SUser: res.AppParameters.SystemDetail.SUser,
            SystemDescription: res.AppParameters.SystemDetail.SystemDescription,
            Manufacturer: res.AppParameters.SystemDetail.Manufacturer,
            Model: res.AppParameters.SystemDetail.Model,
            OperatingSystem: res.AppParameters.SystemDetail.OperatingSystem,
            OSRelease: res.AppParameters.SystemDetail.OSRelease,
            DatabaseName: res.AppParameters.SystemDetail.DatabaseName,
            DatabaseRelease: res.AppParameters.SystemDetail.DatabaseRelease,
            PersonIncharge: res.AppParameters.SystemDetail.PersonIncharge,
            SIContactNo: res.AppParameters.SystemDetail.SIContactNo,
            SIEmail: res.AppParameters.SystemDetail.SIEmail,
          };        
          ...
          if(res.OnBehalfOf){
            this.supModel.Employee =                
            {
              ...
              EmpCompanyName: res.Employee.EmpCompanyName,
              EmpCompany1stAddress: res.Employee.EmpCompany1stAddress,
              Emp1stEmail: res.Employee.Emp1stEmail,
              Emp1stMobile: res.Employee.Emp1stMobile,
              EmpDesignation: res.Employee.EmpDesignation
            };
            
            this.parsedRequestInfo.Employee = this.supModel.Employee;
          }
          
          if(res.PendingTo.length >0 ){

            res.PendingTo.forEach(pen => {
        ...
        //===passing ngif values===
        if(this.supModel.AppParameters.PriorityInfo.Priority = 'Highest'){
          this.showHPBusImpactDiv = true;
        }
        //this._form.value.AppParameters.PriorityInfo.Priority = 'Highest';
        ...
        let masterListInfo:any = this.parsedRequestInfo;
        ...
        if(Object.prototype.hasOwnProperty.call(masterListInfo, 'AppParameters')){ 
          ...
          if(this.requestorsInfo.CustId != ""){
            //this.showReqInfoDiv = true;     
            //=== loading data ===
            //this._form.controls.Requestor.patchValue(this.supModel.Requestor);
    ...
    async onRequestForSelect(e:any){
      ...
      // supportCategories.forEach((sc:any) => {
      //   //SystemType
      // });
    ...
    async onReqForCategorySelect(e:any){
      if(e == "SAP"){
        this.requestForCategories = this.sapModules;
      }else{
        this.requestForCategories = this.otherCategories;
        ...
        try{
              ...
              .subscribe(
                (res:any)=>{
                  ...
                  // this.supModel.AppParameters.ProcessLogs =                
                  // {                    
                  //   Title: res.value[0].Title,
                  //   Created: res.value[0].Created,        
                  //   Status: res.value[0].Status,
                  //   ProcessByName: res.value[0].ProcessByName,
                  //   ProcessByEmail: res.value[0].ProcessByEmail,
                  //   Comments: res.value[0].Comments,
                  // };
    ...
    private _getSupCatResponsibles(title:any){
      ...
      return new Promise((resolve, reject)=>{
        try{ 
          ...
          //this.httpClient.get(`https://bergerpaintsbd.sharepoint.com/sites/BergerTech/_api/web/lists/getByTitle('SupportCategoryResponsibles')/items?&$top=2000&$select=ID,Category,SubCategory,Modules,Incharge/ID,Incharge/Title,Incharge/EMail,From,To&$expand=Incharge/ID&$filter=To ge '${new Date().toISOString()}'`) 
          .subscribe(
            (res:any)=>{
              if(res.value.length > 0){                
