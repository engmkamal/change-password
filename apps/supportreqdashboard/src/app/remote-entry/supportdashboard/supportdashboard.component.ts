...
@Component({
  selector: 'portal-supportdashboard',
  templateUrl: './supportdashboard.component.html',
  styleUrls: ['./supportdashboard.component.scss'],
})
export class SupportdashboardComponent implements OnInit, AfterViewInit {
  ...
  dashboardGridDef(){   
    return new Promise((resolve, reject)=>{
      ...
      this.mpTG.rowGroupPanelShow = 'always'; //'onlyWhenGrouping';
  ...
  async ngOnInit() {
    ...
    let pendingList = {
      apiUrl: `https://bergerpaintsbd.sharepoint.com/sites/BergerTech/_api/web/lists/getByTitle('supportmaster')/items?&$top=100&$select=*&$orderby=Created desc`,      
    ...
    .subscribe(
      (items:any) => {
        ...
        for(let i=0; i<items.value.length; i++){
            ...
            if(sapReq.PendingTo.length >0){
              let aprv = '';
              for(let j=0; j<sapReq.PendingTo.length; j++){
                aprv = aprv + '  ' + sapReq.PendingTo[j].approversName;
              }
              sapReq.AllPendingTo = aprv;
            }else{
              sapReq.AllPendingTo = "";
            }
    ...
    const dbListsInfoUrl = "https://bergerpaintsbd.sharepoint.com/sites/BergerTech/Style Library/bergertechportal/assets/supdashboardslistsinfo.ts";
