...
// @Component({
//   selector: 'portal-mynonsapworkflow',
//   templateUrl: './mynonsapworkflow.component.html',
//   styleUrls: ['./mynonsapworkflow.component.scss'],
// })
// export class MynonsapworkflowComponent implements OnInit {
//   constructor() {}
...
export class MynonsapworkflowComponent implements OnInit, AfterViewInit {
          ...
          this.httpClient.get<any[]>(apiUrl)
            .subscribe(
              (items:any) => {                
                ...
                if(this.rowData.length < this.mpTG.paginationPageSize){
                  this.tblContentHeight = (this.rowData.length * 20) + 100;
                }
  ...
  async ngOnInit() {
    ...
    let pendingList = {
      apiUrl: `https://bergerpaintsbd.sharepoint.com/sites/BergerTech/_api/web/lists/getByTitle('supportmaster')/items?&$top=100&$select=*&$filter=((CustId eq ${cid}) and (SystemType eq 'Others'))&$orderby=Created desc`,      
    ...
    .subscribe(
      (items:any) => {
        ...
        for(let i=0; i<items.value.length; i++){
          ...
          if( items.value[i].SystemType == 'Others'){
            let sapReq = {
